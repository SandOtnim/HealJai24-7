...
### 📋 มติประชุม 10 มิ.ย. 2026 (โน้ตทีม — override ของเก่าถ้าขัดกัน)
- **สวนหย่อนใจ:** เลือก howto random กิจกรรม (random แบบไหน) · เอา "เขียนเอง" ออก · เอา "ช้อปปิ้งทิพย์" ออก
- **บ้านพักใจ:** สมาธิ เพิ่มเสียงการหายใจ · แมวให้ดูสมจริง (แซนลองก่อน — ไม่ทันใส่ under construction) · ห้องเช็คใจทำเป็น mood tracker · ระบายสีอยากให้สวยกว่านี้
- **ศาลฮีลใจ:** คำทำนาย/เซียมซี (แท้งค์ขัดกลอน → แปล EN → Sheet แท็บ mu)
- **คาเฟ่รวมใจ:** คุยกัน เอาออก · หนัง/ซีรีส์ เอาออก · บทความฮีลใจ — เช็คลิขสิทธิ์ว่าเอาของคนอื่นมาได้ไหม · แบบทดสอบของมุก · ดูดวง — ดีไซน์ไพ่ · โต๊ะทำงาน: บรรยากาศ-เลือกเสียงแยกให้ชัด เอาบรรยากาศเป็น first theme ที่เลือกได้ · ทำภาพรวมดีไซน์ให้เห็นว่าเป็นสถานที่แบบห้องอื่นๆ · ตัด notice 1323
- **เกาะแห่งเวลา (เกาะฮีลใจ):** ปิดไปก่อน · **ห้องระบาย:** เอาออก · **ร้านไว้ใจ:** เอาออก
- **รวมๆ:** ทำภาพ bg ทุกห้องให้คุณภาพเดียวกับพิพิธภัณฑ์+ห้องนอน
- **อื่นๆ:** หาวิธี log in ⚠️ (กลับคำมติเดิมที่ตัด login ออก — ยังไม่สรุปวิธี อย่าเพิ่ง implement จนกว่าจะคุยกัน)
- **หลังบ้าน Google Sheet:** ในประชุมเชื่อมไม่สำเร็จ (ชีทคนละตัว/owner ของชีท/EN ไม่ขึ้น) — ต้องทำใหม่กับแซน
- นัดถัดไป (จาก transcript ถอดเสียง ไม่ 100%): 22 มิ.ย. 20:00 — รอ owner ยืนยัน รวมถึงว่า deadline 14 มิ.ย. เลื่อนตามหรือไม่
...
- คำเซียมซี rewrite ให้โอบกอดตัวเอง + (มติประชุม) ยาวขึ้น เป็นกลอน 8 เกิน 20 ใบ มีคำทำนาย ใบเซียมซีจริงมีเซนส์ — เนื้อหารอ "แท้งค์"
  - 5 มิ.ย.: มี research `research-siamsi.md` (เซียมซีจริง 6 สำนัก + แบบโมเดิร์น) + ร่าง AI 24 ใบ `siamsi-heal-draft.md` (รอแท้งค์ขัด แล้วแปล EN ครบ → วางแท็บ `mu` ใน Google Sheet) + skill `siamsi-writing` (ผู้ใช้เซฟเอง)
...
- 6 มิ.ย.: ทำเดโมวงล้อใหม่ (ภาพ clay 3D ปั่นจาก Higgsfield → คัทโปร่ง `assets/images/garden-wheel.png` · เดโม `team/bakeoff/garden-wheel-demo.html` หมุนเนียน 4.4 วิ ลูกศรนิ่งบน หยุดถูกช่อง การ์ดผล 6 กิจกรรม 2 ภาษา) — **owner ยังไม่ชอบ พักไว้ก่อน** (ยังไม่เอาเข้า travel.html จริง)
...
**🎨 Art Direction (ล็อก 6 มิ.ย. — owner เป็นผู้กำกับซีรีส์ บาร์ภาพสูงมาก ต้อง "ว้าว")**
- สไตล์ภาพเว็บ = **3D ดินปั้น (clay diorama) มุมไอโซเมตริก พาสเทล พื้นครีม** (เหมือน garden.jpg, heart-map.jpg) — ทั้งเว็บต้องชุดเดียวกัน
- Workflow ภาพ: **Pinterest ตั้งทิศ → Higgsfield ปั่น (Auto + Unlimited ผ่าน God Mode extension) → คัทพื้นเป็น PNG โปร่ง** (วิธีคัท: flood-fill/วงกลม mask ด้วย PIL ใน sandbox)
- ⚠️ ดึงไฟล์ PNG/รูปจาก Higgsfield เข้า sandbox ตรงๆ **ไม่ได้** (ระบบบล็อก base64/URL) → ต้องให้ owner กด Download + เซฟเข้าโฟลเดอร์ asset เอง
- **SVG / Recraft 4.1:** Higgsfield มี Recraft ปั่น SVG เวกเตอร์ได้แล้ว — เหมาะกับ **ของแบน: ไอคอน/ลายเส้น/มาสคอต flat** (คมทุกขนาด แก้สี/animate ง่าย) · SVG = ข้อความ น่าจะดึงเข้าโปรเจกต์ตรงๆ ได้ (เลี่ยงบล็อก) — **ยังไม่ได้เทส** · ภาพดินปั้น (แมว/ห้อง/สวน) ยังคงใช้ PNG เหมือนเดิม (SVG ทำแสง/พื้นผิวดินปั้นไม่ได้)

**👥 Head ประจำโปรเจกต์ (ทดลอง 6 มิ.ย. — owner เลือกอยู่)**
- `team/HEAD-A-สายลุย.md` (ลงมือก่อน ให้ของเสร็จ) vs `team/HEAD-B-สายปรึกษา.md` (เสนอแผน+ทางเลือกก่อน) · ผลประชันงานสวนหย่อนใจอยู่ใน `team/bakeoff/` · ทั้งคู่: ไม่ push เอง owner เคาะเสมอ
